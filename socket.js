...
module.exports = (server, app) => {
	...
	io.use(function (socket, next) {
		// 토큰이 있을 경우에만 작동이 한다.
		if (socket.handshake.query.brandName) {
			jwt.verify(
				socket.handshake.query.brandName,
		...
		console.log('뭐가 있는지 확인하기 위한 ', socket.decoded);
		...
		socket.on('inspectionBid', data => {
			...
			} else if (app.get('resentPrice') <= data.price) {
				// 가격이 더 크거나 같으면 올라간 가격을 제공
